...
const CaseCard: React.FC<CaseCardProps> = ({ searchResult, onClick, isFavorite, onToggleFavorite }) => {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow cursor-pointer border border-gray-200 p-3"
    >
      {/* Header with title, favorite button, and relevance score */}
      <div className="flex justify-between items-start mb-2">
        <h3 className="text-sm font-semibold text-gray-900 flex-1 mr-3 leading-tight">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-2 text-xs text-gray-600">
        ...
        <div className="flex items-center">
          ...
          {[caseData.region, caseData.prefecture, caseData.city].filter(Boolean).join(' > ')}
        </div>
